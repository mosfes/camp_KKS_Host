...
export async function requireTeacher() {
    ...
    if (!teacher) return { teacher: null, error: Response.json({ error: "กรุณาเข้าสู่ระบบก่อน" }, { status: 401 }) };
...
export async function requireStudent() {
    ...
    if (!student) return { student: null, error: Response.json({ error: "กรุณาเข้าสู่ระบบก่อน" }, { status: 401 }) };
