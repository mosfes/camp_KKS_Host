...
export default function AttendanceModal({ isOpen, onClose, campId, campName }: AttendanceModalProps) {
  const { showError, showConfirm } = useStatusModal();
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                  ...
                  <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-3">
                    ...
                    <button onClick={() => fetchResults()} disabled={resultsLoading}
                      className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg text-gray-500 bg-gray-100 hover:bg-gray-200 transition-colors disabled:opacity-50 h-[32px]">
                      <RefreshCw size={13} className={resultsLoading ? "animate-spin" : ""} />รีเฟรช
                    </button>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    {filteredResults.map(result => (
                      <div key={result.enrollmentId}
                        ...
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${result.isCheckedIn ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}>
                          <User size={20} />
                        </div>
                        ...
                        {result.isCheckedIn ? (
                          ...
                          <div className="flex items-center gap-1.5 bg-gray-100 text-gray-500 px-3 py-1.5 rounded-full text-xs font-medium shrink-0">
                            <XCircle size={14} />ยังไม่เช็คชื่อ
                          </div>
                        )}
                      </div>
                    ))}
