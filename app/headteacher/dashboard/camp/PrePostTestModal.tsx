          ...
          <>
            <ModalHeader className="flex flex-col gap-1 p-6 border-b border-gray-100">
              <div className="flex items-center gap-2 text-[#6b857a]">
                <BarChart2 size={24} />
                <h2 className="text-2xl font-bold text-gray-900">เปรียบเทียบคะแนนก่อนเรียน - หลังเรียน</h2>
              </div>
            ...
            <ModalBody className="p-6 bg-gray-50/50">
              {loading ? (
                ...
                <div className="text-center py-12 text-gray-500">
                  <p className="font-medium text-lg text-gray-900 mb-1">ไม่พบข้อมูลแบบทดสอบ</p>
                  ...
                  {data.map((pair: any, index: number) => {
                    ...
                    const filteredScores = pair.studentScores.filter((s: any) => {
                      if (searchQuery) {
                        ...
                        if (!s.studentName.toLowerCase().includes(q) && !String(s.studentId).includes(q)) return false;
                      }
                    ...
                    return (
                      <div key={index} className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                        <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                          <div>
                            <h3 className="font-bold text-gray-900 text-lg">ฐาน: {pair.stationName}</h3>
                            <p className="text-sm text-gray-500">
                              ก่อนเรียนเต็ม {pair.preTest.total} คะแนน · หลังเรียนเต็ม {pair.postTest.total} คะแนน
                            </p>
                          </div>
                        </div>
                          ...
                          <table className="w-full text-left border-collapse">
                            <thead>
                              <tr className="bg-gray-50 border-b border-gray-100 text-gray-600 text-sm">
                                ...
                                <th className="px-6 py-3 font-semibold text-center w-32">ก่อนเรียน</th>
                                <th className="px-6 py-3 font-semibold text-center w-32">หลังเรียน</th>
                                <th className="px-6 py-3 font-semibold text-center w-32">พัฒนาการ</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100">
                              {filteredScores.length === 0 ? (
                                <tr>
                                  <td colSpan={5} className="py-8 text-center text-gray-500">ไม่พบนักเรียนที่ค้นหา</td>
                                </tr>
                              ) : (
                                filteredScores.map((s: any) => (
                                  <tr key={s.studentId} className="hover:bg-gray-50/50 transition-colors">
                                    <td className="px-6 py-3 text-sm text-gray-600">{s.studentId}</td>
                                    <td className="px-6 py-3 text-sm font-medium text-gray-900">{s.studentName}</td>
                                    ...
                                    <td className="px-6 py-3 text-sm text-center">
                                      {s.diff !== null ? (
                                        <div className={`inline-flex flex-col items-center justify-center px-3 py-1.5 rounded-lg border ${s.diff > 0
                                            ? 'bg-green-50 text-green-700 border-green-200'
                                            : s.diff < 0
                                              ? 'bg-red-50 text-red-700 border-red-200'
                                              : 'bg-gray-50 text-gray-600 border-gray-200'
                                          }`}>
                                          <span className="font-bold">{s.diff > 0 ? `+${s.diff}` : s.diff}</span>
                                          {pair.postTest.total > 0 && (
                                            <span className="text-[10px] font-medium opacity-80 mt-0.5">
                                              ({s.diff > 0 ? '+' : ''}{Math.round((s.diff / pair.postTest.total) * 100)}%)
                                            </span>
                                          )}
                                        </div>
                                      ) : <span className="text-gray-400">-</span>}
