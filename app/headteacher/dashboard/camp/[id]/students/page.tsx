...
import { ChevronLeft, Search, AlertCircle, Users, Activity, FileText } from "lucide-react";
...
export default function CampStudentsPage() {
  ...
  const fetchStudents = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/camps/${campId}/students?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}&filter=${filter}`);
      if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-[#F5F1E8]">
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl px-10 py-8 flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              <p className="font-semibold text-[#2d3748] text-base">กำลังโหลดข้อมูลนักเรียน</p>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900 mb-8">ข้อมูลนักเรียนในค่าย</h1>
        ...
        {summary && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <p className="text-2xl font-bold text-gray-900 mb-2">{summary.totalStudents} <span className="text-sm font-normal text-gray-500">คน</span></p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-blue-100">
              ...
              <p className="text-2xl font-bold text-gray-900 mb-2">{summary.remarksCount} <span className="text-sm font-normal text-gray-500">คน</span></p>
              <div className="flex flex-wrap gap-2 mt-2">
                {summary.remarks.slice(0, 3).map((r, i) => (
                  <Chip key={i} size="sm" className="bg-blue-50 text-blue-700 max-w-full" style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.text}</Chip>
                ))}
                {summary.remarksCount > 3 && <span className="text-xs text-gray-500 mt-1">+{summary.remarksCount - 3}</span>}
              </div>
            </div>
          </div>
        )}
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              <div className="w-72">
                <Input
                  ...
                  className="w-full"
                  size="sm"
                  classNames={{ inputWrapper: "h-10 border border-gray-100 bg-white" }}
                />
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 text-gray-600 text-sm border-y border-gray-100">
                  ...
                  <th className="p-4 font-semibold rounded-tr-lg">เงื่อนไขพิเศษ/อื่นๆ</th>
                </tr>
              </thead>
              <tbody>
                {!loading && students.length === 0 ? (
                  ...
                  students.map((row, idx) => (
                    <tr key={idx} className="border-b border-gray-50 hover:bg-gray-50 transition-colors text-sm">
                      <td className="p-4 text-gray-900 font-medium">{row.student.students_id}</td>
                      <td className="p-4 text-gray-900">
                        {row.student.prefix_name || ''} {row.student.firstname} {row.student.lastname}
                      </td>
                      <td className="p-4 text-gray-600">{row.student.tel || '-'}</td>
                      <td className="p-4">
                        {row.student.food_allergy && row.student.food_allergy !== '-' && row.student.food_allergy !== 'ไม่มี' ? (
                          <Chip size="sm" className="bg-red-50 text-red-700 border border-red-100">{row.student.food_allergy}</Chip>
                        ) : (
                      ...
                      <td className="p-4">
                        {row.student.remark && row.student.remark !== '-' && row.student.remark !== 'ไม่มี' ? (
                          <span className="text-blue-700 font-medium">{row.student.remark}</span>
                        ) : (
                          <span className="text-gray-400">-</span>
                        )}
                      </td>
