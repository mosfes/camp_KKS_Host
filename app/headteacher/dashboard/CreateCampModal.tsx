...
export default function CreateCampModal({
  ...
  useEffect(() => {
    if (isOpen && projectType === "continuing" && templateData) {
      ...
      const dailySchedule =
        campSource.camp_daily_schedule &&
        Array.isArray(campSource.camp_daily_schedule)
      ...
      setShirtImageFiles([null, null, null]);
      setCampImage(null);
  ...
  useEffect(() => {
    if (
      ...
      if (
        campSource.camp_classroom &&
        Array.isArray(campSource.camp_classroom)
      ) {
        const classroomIds = campSource.camp_classroom
          .map(
            (cc: any) =>
              cc.classroom_classroom_id ||
              cc.classroom_id ||
              (cc.classroom && cc.classroom.classroom_id),
        ...
        console.log(
          "Setting classroom IDs from camp_classrooms:",
          classroomIds,
        );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.saveAsTemplate && !formData.templateName.trim()) {
      showWarning(
        "ข้อมูลไม่ครบถ้วน",
        "กรุณาระบุชื่อ Template หากต้องการบันทึกเป็น Template",
      );
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-5 sm:p-6 border-b bg-white flex justify-between items-center">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900">
              {projectType === "continuing"
                ? "สร้างค่ายจาก Template"
                : "สร้างค่ายใหม่"}
            </h2>
            <p className="text-xs sm:text-sm text-gray-500">
              {projectType === "continuing"
                ? "ตรวจสอบและแก้ไขข้อมูลจาก Template ตามต้องการ"
                : `กรอกข้อมูลค่ายเพื่อเริ่มต้น (${steps[currentStep - 1].title})`}
            </p>
          </div>
          <button
        ...
        <div className="overflow-y-auto p-5 sm:p-6 flex-1">
          ...
          {/* STEP 3: เสื้อค่าย & บันทึก Template */}
          {currentStep === 3 && (
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2 mb-1">
                  <span className="w-1.5 h-6 bg-[#6b857a] rounded-full" />
                  เสื้อค่ายและการตั้งค่า
                </h3>
                <p className="text-xs text-gray-500">
                  เลือกเปิดหรือปิดการรับจองเสื้อ อัปโหลดตัวอย่างเสื้อค่าย และตั้งค่า Template
                </p>
              </div>
                ...
                <div className="space-y-6 p-5 border border-gray-200 rounded-2xl bg-gray-50/50">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-600 uppercase mb-1">
                      รูปตัวอย่างเสื้อค่าย (สูงสุด 3 รูป)
                    </label>
        ...
        <div className="p-4 sm:p-5 border-t bg-gray-50 flex items-center justify-between gap-3">
          {currentStep === 1 ? (
            <button
              className="px-5 sm:px-6 py-2.5 sm:py-3 bg-white text-gray-700 border border-gray-300 rounded-xl hover:bg-gray-100 transition-all font-semibold text-sm"
              disabled={isLoading}
              onClick={onClose}
              type="button"
            >
            ...
            <button
              className="px-5 sm:px-6 py-2.5 sm:py-3 bg-white text-gray-700 border border-gray-300 rounded-xl hover:bg-gray-100 transition-all font-semibold flex items-center gap-1.5 text-sm"
              disabled={isLoading}
          ...
          {currentStep < 3 ? (
            ...
            >
              {isLoading ? (
                ...
                <>
                  <Plus size={16} />
                  <span>สร้างค่ายและดำเนินการต่อ</span>
                </>
              )}
            </button>
          )}
