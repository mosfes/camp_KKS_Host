...
import { ChevronLeft, Target, FileText, CheckCircle2, Circle } from "lucide-react";
...
export default function StudentStationDetailPage() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (foundCamp) {
                    ...
                    const foundStation = foundCamp.station?.find((s: any) => s.station_id === Number(stationId));
                    if (foundStation) {
    ...
    const submitMission = async () => {
        ...
        const payloadAnswers = Object.entries(answers).map(([qid, val]) => {
            ...
            const q = selectedMission.mission_question.find((mq: any) => mq.question_id === Number(qid));
            return {
    ...
    const isMissionCompleted = (missionId: number) => {
        ...
        // status='completed'
        return camp.missionResults.some((r: any) =>
    ...
    if (!station) return <div className="p-8 text-center bg-[#F5F1E8] min-h-screen">ไม่พบฐานกิจกรรม</div>;
        ...
        <div className="min-h-screen bg-[#F5F1E8]">
            ...
            <div className="max-w-2xl mx-auto px-4 py-8 space-y-4">
                ...
                {station.mission?.length === 0 && (
                    <div className="text-center text-gray-400 py-10">ยังไม่มีภารกิจในฐานนี้</div>
                )}
                        ...
                        >
                            <div className="flex justify-between items-start">
                                <div className="flex items-start gap-3">
                                    <div className={`mt-1 ${completed ? 'text-green-600' : 'text-gray-400'}`}>
                                        {completed ? <CheckCircle2 size={20} /> : <Circle size={20} />}
                                    </div>
                                    <div>
                                        <h3 className={`font-bold ${completed ? 'text-green-800' : 'text-gray-800'}`}>
                                ...
                                {completed && <span className="text-xs font-bold text-green-600 bg-green-100 px-2 py-1 rounded-full">สำเร็จ</span>}
                            </div>
                        </div>
                    );
                })}
                        ...
                        <>
                            ...
                            <ModalBody className="py-6 space-y-6">
                                ...
                                <div className="space-y-6">
                                    {selectedMission?.mission_question?.map((q: any, idx: number) => (
                                            ...
                                            {q.question_type === "MCQ" && (
                                                <div className="space-y-2">
                                                    {q.choices?.map((c: any) => (
                                                        <div
                                                            key={c.choice_id}
                                                            onClick={() => handleAnswerChange(q.question_id, c.choice_text)}
                                                            className={`
                                                        ...
                                                        >
                                                            ...
                                                            `}>
                                                                {answers[q.question_id] === c.choice_text && <div className="w-2.5 h-2.5 rounded-full bg-white" />}
                                                            </div>
