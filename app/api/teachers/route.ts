...
export async function POST(req) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          count: newTeachersList.length + restoredTeachersList.length,
        },
        { status: 201 },
      );
    }
    ...
    return NextResponse.json(
      { _error: "เพิ่มข้อมูลครูไม่สำเร็จ" },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    if (owningClassroom) {
      return NextResponse.json(
        {
          error:
            "ไม่สามารถลบได้: ครูท่านนี้เป็นครูประจำชั้น (ต้องเปลี่ยนครูประจำชั้นก่อน)",
        },
        { status: 400 },
      );
    }
    ...
    if (owningCamp) {
      return NextResponse.json(
        {
          error:
            "ไม่สามารถลบได้: ครูท่านนี้เป็นผู้สร้างค่าย (ต้องลบค่ายหรือโอนสิทธิ์ก่อน)",
        },
        { status: 400 },
      );
    }
...
export async function PUT(req) {
  try {
    ...
    return NextResponse.json(
      { _error: "แก้ไขข้อมูลครูไม่สำเร็จ" },
      { status: 500 },
    );
  }
}
