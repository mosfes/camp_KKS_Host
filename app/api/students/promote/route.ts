...
export async function GET(req) {
  try {
    ...
    for (const room of sourceClassrooms) {
      ...
      if (!groupedData.has(key)) {
        ...
        const targetRoom = targetClassrooms.find(
          (tr) =>
            tr.grade === nextGrade && tr.type_classroom === room.type_classroom,
        );
        ...
        if (targetRoom) {
          ...
          if (
            targetRoom.classroom_teacher &&
            targetRoom.classroom_teacher.length > 0
          ) {
          ...
          const matchingNextGradeRooms = sourceClassrooms.filter(
            (r) =>
              r.grade === nextGrade && r.type_classroom === room.type_classroom,
          );
          ...
          if (matchingNextGradeRooms.length > 0) {
            for (const nextRoom of matchingNextGradeRooms) {
              ...
              // Secondary Teachers
              if (
                nextRoom.classroom_teacher &&
                nextRoom.classroom_teacher.length > 0
              ) {
    ...
    return NextResponse.json({ _error: "Failed to preview" }, { status: 500 });
  }
}
...
export async function POST(req) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      for (const item of plan) {
        ...
        if (item.isNewRoom && teacherIds.length === 0) {
          throw new Error(
            `กรุณาเลือกครูประจำชั้นสำหรับห้อง ${item.targetGrade} (${item.targetType})`,
          );
        }

        const primaryTeacherId =
          teacherIds.length > 0 ? parseInt(teacherIds[0]) : null;
        const secondaryTeacherIds = teacherIds
          .slice(1)
          .map((id) => parseInt(id));
    ...
    return NextResponse.json(
      { _error: "เกิดข้อผิดพลาดในการบันทึก" },
      { status: 500 },
    );
  }
}
