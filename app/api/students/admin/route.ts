...
export async function PATCH(request) {
    ...
    try {
        ...
        if (!existing) return NextResponse.json({ error: "ไม่พบนักเรียนในถังขยะ" }, { status: 404 });
...
export async function DELETE(request) {
    ...
    try {
            ...
            if (enrollments.length > 0) {
                ...
                if (missionResults.length > 0) {
                    ...
                    if (answers.length > 0) {
                        ...
                        await tx.mission_answer_mcq.deleteMany({ where: { mission_answer_id: { in: aIds } } });
                        await tx.mission_answer_text.deleteMany({ where: { mission_answer_id: { in: aIds } } });
                        await tx.mission_answer_photo.deleteMany({ where: { mission_answer_id: { in: aIds } } });
                        await tx.mission_answer.deleteMany({ where: { answer_id: { in: aIds } } });
                    }
                    await tx.mission_result.deleteMany({ where: { mission_result_id: { in: mrIds } } });
                }
                ...
                if (evaluations.length > 0) {
                    ...
                    if (evalAnswers.length > 0) {
                        ...
                        await tx.suggestion_analysis_summary.deleteMany({ where: { evaluation_answer_evaluation_id: { in: eaIds } } });
                        await tx.evaluation_answer.deleteMany({ where: { answer_id: { in: eaIds } } });
                    }
                    await tx.evaluation.deleteMany({ where: { evaluation_id: { in: evIds } } });
                }

                await tx.attendance_record_student.deleteMany({ where: { student_enrollment_student_enrollment_id: { in: eIds } } });
                await tx.certificate.deleteMany({ where: { student_enrollment_id: { in: eIds } } });
                await tx.student_enrollment.deleteMany({ where: { student_enrollment_id: { in: eIds } } });
            }

            await tx.classroom_students.deleteMany({ where: { student_students_id: id } });
        ...
        return NextResponse.json({ error: "ลบถาวรไม่สำเร็จ" }, { status: 500 });
    }
}
