...
export async function GET(request, { params }) {
    try {
        ...
        for (const station of stations) {
            ...
            if (preTests.length > 0 && postTests.length > 0) {
                ...
                const studentScores = enrollments.map(enroll => {
                    ...
                    const preResult = preTest.mission_result.find(r => r.student_enrollment_id === enroll.student_enrollment_id && r.status === 'completed');
                    let preScore = null;
                    if (preResult) {
                        preScore = 0;
                        for (const ans of preResult.mission_answer) {
                            // find if correct
                            const mq = preTest.mission_question.find(q => q.question_id === ans.mission_question_question_id);
                            if (mq) {
                    ...
                    const postResult = postTest.mission_result.find(r => r.student_enrollment_id === enroll.student_enrollment_id && r.status === 'completed');
                    let postScore = null;
                    if (postResult) {
                        postScore = 0;
                        for (const ans of postResult.mission_answer) {
                            // find if correct
                            const mq = postTest.mission_question.find(q => q.question_id === ans.mission_question_question_id);
                            if (mq) {
                                const chosenLetter = ans.answer_mcq?.[0]?.question_text;
                                if (chosenLetter) {
                                    const charCode = chosenLetter.charCodeAt(0) - 65;
                    ...
                    return {
                        ...
                        preScore,
                        postScore,
                        diff: (preScore !== null && postScore !== null) ? (postScore - preScore) : null
                    };
                });
        ...
        return NextResponse.json({ error: 'Failed to fetch pre-post tests' }, { status: 500 });
    }
}
