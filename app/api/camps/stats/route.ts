...
export async function GET() {
  ...
  try {
    ...
    const allowedCampIds = rawCamps
      .filter((camp) => {
        if (camp.created_by_teacher_id === teacherId) return true;
        if (
          camp.teacher_enrollment?.some(
            (t) => t.teacher_teachers_id === teacherId,
          )
        )
          return true;
        ...
        camp.camp_classroom?.forEach((cc) => {
          if (cc.classroom?.teachers_teachers_id === teacherId)
            isHomeroom = true;
          if (
    ...
    const surveyResponseRate =
      totalEnrollments > 0
        ? Math.round((totalSurveyResponses / totalEnrollments) * 100)
        : 0;
