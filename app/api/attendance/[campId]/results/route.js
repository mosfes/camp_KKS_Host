...
export async function GET(request, { params }) {
    ...
    const memStore = globalThis._campAttendanceRecords ?? (globalThis._campAttendanceRecords = new Map());
    ...
    results.sort((a, b) => {
        ...
        if (!a.isCheckedIn && b.isCheckedIn) return 1;
        if (a.isCheckedIn && b.isCheckedIn) return new Date(b.checkedAt) - new Date(a.checkedAt);
        return a.studentId - b.studentId;
    });

    return NextResponse.json({ results, totalCheckedIn: results.filter(r => r.isCheckedIn).length, total: results.length });
}
...
export async function DELETE(request, { params }) {
    ...
    if (roundId) {
        ...
        [...memStore.keys()].filter(k => k.startsWith(`${cid}:`)).forEach(k => memStore.delete(k));
        const roundsStore = globalThis._campAttendanceRounds ?? new Map();
        roundsStore.delete(cid);
    }

    return NextResponse.json({ success: true, message: 'ล้างข้อมูลเช็คชื่อแล้ว' });
}
